...
def _grades_to_snapshot(grades: dict) -> dict:
    ...
    for subject, rows in grades.items():
        snap[subject] = [
            {"current_score": r.get("current_score"), "max_score": r.get("max_score")}
            for r in rows
        ]
    return snap
            ...
            if i < len(old_rows):
                ...
                if old_score != new_score:
                    if old_score is None and new_score is not None:
                        changes.append(f"📥 *{subject}* — КТ{i+1}: появилась оценка *{new_score}*")
                    elif old_score is not None and new_score is None:
                        changes.append(f"🗑 *{subject}* — КТ{i+1}: оценка удалена (было {old_score})")
                    else:
                        changes.append(f"✏️ *{subject}* — КТ{i+1}: {old_score} → *{new_score}*")
            else:
                if new_score is not None:
                    changes.append(f"📥 *{subject}* — КТ{i+1}: новая оценка *{new_score}*")
    for subject in new:
...
class GradeMonitor:
    ...
    def start(self, user_id: int):
        ...
        asyncio.create_task(self._storage.set_monitor_config(user_id, active=True))
    ...
    def stop(self, user_id: int):
        ...
        if task and not task.done():
            task.cancel()
        asyncio.create_task(self._storage.set_monitor_config(user_id, active=False))
        logger.info("Monitor stopped for user %d", user_id)

    async def restore_active_monitors(self):
        user_ids = await self._storage.all_user_ids()
        for uid in user_ids:
            cfg = await self._storage.get_monitor_config(uid)
            if cfg.get("active"):
    ...
    async def _monitor_loop(self, user_id: int):
        ...
        while True:
            try:
                cfg           = await self._storage.get_monitor_config(user_id)
                ...
                if _is_in_window(windows):
                    creds = await self._storage.get_credentials(user_id)
                    if creds:
                        session_ok = False
                        if parser:
                            try:
                        ...
                        if not session_ok:
                            ...
                            if not session_ok:
                                logger.warning("Re-login failed for user %d", user_id)
                                await parser.close()
                                parser = None
                        ...
                        if session_ok and parser:
                            try:
                                ...
                                old_snap = await self._storage.get_grades_snapshot(user_id)

                                changes = _diff_grades(old_snap, new_snap)
                                if changes:
                                    text = "🔔 *Изменения в оценках!*\n\n" + "\n".join(changes)
                                    await self._bot.send_message(user_id, text, parse_mode="Markdown")
                                    await self._storage.save_grades_snapshot(user_id, new_snap)
                                    logger.info("Sent %d changes to user %d", len(changes), user_id)
                                elif not old_snap:
                                    await self._storage.save_grades_snapshot(user_id, new_snap)
                                    logger.info("Saved initial snapshot for user %d", user_id)

                                await self._storage.set_last_check(user_id, _time.time())
                            except Exception as e:
                                logger.error("Grade fetch error for %d: %s", user_id, e)
                else:
                    logger.debug("User %d outside time window, skipping", user_id)
